...
function saveResults(outputPath) { 
	...
	fileName = "/SummaryCellCount_" + prefix + "_" + day+month+year;
...
function processFile(input, output, file) {
	...
	if(file.contains("Nissl")) {
		...
		if(!file.contains("Control")) {
			...
			print("Invoke Count_Cells_Control: " + file);
			runMacro(macros + File.separator + "Count_Cells_Control.ijm");
		}
	}
}